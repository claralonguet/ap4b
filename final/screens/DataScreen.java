...
public class DataScreen extends GameScreen{
    ...
    private void createBoxesArray(){
        ...
        for(int i = 0; i < 3; i++){
            ...
            for (int j = 0; j < 5; j++) {
                ...
                cb.setSelected(joueur.getData().getNumbers().get(i).get(j));

                cb = modifyFontWhenChecked(cb);
    ...
    private class CheckBoxListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e){
            ...
            s = modifyFontWhenChecked(s);

            s.repaint();
            s.revalidate();

            //Met à jour les données du joueur
            int col = 0; int n = 5 - Integer.parseInt(s.getText());
            for (int i = 0; i < checkBoxesArray.size(); i++) {
                for (JCheckBox c : checkBoxesArray.get(i)) {
                    if(c.equals(s)){
                        col = i;
                    }
                }
            }
            joueur.majNumber(s.isSelected(), n, col);
        }        
    }

    private JCheckBox modifyFontWhenChecked(JCheckBox cb){
